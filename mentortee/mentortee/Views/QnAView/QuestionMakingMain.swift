    ...
    var body: some View {
        ...
        // 버튼을 누를 시에,
                trailing: Button(action: {
                    firestoreData.userQuestionList.append(QuestionData(id: Auth.auth().currentUser?.uid ?? "", nickname: userInfo.myPageData.username, question: myQuestion, category: selection.map { $0 }, uploadDate: Date(), myThought: [MyAnswer(thought: "")]))
                    firestoreData.addUserQuestionData(questionData: QuestionData(id: Auth.auth().currentUser?.uid ?? "", nickname: userInfo.myPageData.username, question: myQuestion, category: selection.map { $0 }, uploadDate: Date(), myThought: [MyAnswer(thought: "")]))
                    isDoneAlert = true
                }, label: {
