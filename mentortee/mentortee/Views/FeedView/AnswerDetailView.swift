    ...
    var body: some View {
        VStack {
            HStack(alignment: .top) {
                ...
                }.frame(width: .infinity, alignment: .leading).background(RoundedRectangle(cornerRadius: 10).fill(Color.backgroundColor))
    ...
    var body: some View {
        VStack {
                ...
                ScrollView {
                    ...
                    VStack {
                        ...
                        HStack {
                            Text("9개의 생각이 있어요")
                            Spacer()
                            Image(systemName: self.collapsed ? "chevron.down.circle.fill" : "chevron.up.circle.fill").foregroundColor(Color.mainGreen)
                        }.frame(maxWidth: .infinity, minHeight:50, maxHeight: 50).padding(.horizontal, 10)
                        }.background(RoundedRectangle(cornerRadius: 10).fill(Color.white)).offset(y: collapsed ? 0 : -25)
                    }.frame(maxWidth: .infinity, maxHeight: .infinity, alignment: .top).foregroundColor(Color.black))
            ...
            VStack (alignment: .leading) {
                ...
                }.frame(width: .infinity, height: 40).background(RoundedRectangle(cornerRadius: 20).fill(Color.backgroundColor)).padding(.horizontal, 16)
                HStack {
                    Button(action: { self.checked.toggle() }) {
                        Image(systemName: checked ? "checkmark.circle.fill" : "checkmark.circle").resizable().frame(width: 22, height: 22).foregroundColor(checked ? Color.mainGreen : Color.gray)
                        Text("내 생각 나만 볼래요").font(.system(size: 14)).foregroundColor(checked ? Color.black : Color.gray)
                    }
                }.padding(.leading, 16)
            }.background(Rectangle().fill(Color.white).frame(width: .infinity, height: 85)).shadow(color: .black.opacity(0.1), radius: 10, x: 0, y: -5)
